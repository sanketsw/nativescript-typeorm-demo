...
var QueryBuilder = /** @class */ (function () {
     ...
     */
    QueryBuilder.prototype.getTableName = function (tableName) {
        var _this = this;
        var tablePath = tableName;
        var driver = this.connection.driver;
        var schema = driver.options.schema;
        var metadata = this.connection.hasMetadata(tableName) ? this.connection.getMetadata(tableName) : undefined;
        if (driver instanceof SqlServerDriver_1.SqlServerDriver || driver instanceof PostgresDriver_1.PostgresDriver || driver instanceof MysqlDriver_1.MysqlDriver) {
            if (metadata) {
                if (metadata.schema) {
                ...
                if (metadata.database && !(driver instanceof PostgresDriver_1.PostgresDriver)) {
                    if (!schema && !metadata.schema && driver instanceof SqlServerDriver_1.SqlServerDriver) {
